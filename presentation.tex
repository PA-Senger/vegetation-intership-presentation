...
\begin{frame}{Context}
	\begin{figure}
		\centering
		\includegraphics[width=0.4\textwidth]{images/HIDALGO2-A1-POSTER-copy-6-1.png}
		\caption{Hidalgo2 UBM poster \cite{img:hidalgo2-poster}}
		\label{fig:figure1}
	\end{figure}

	\begin{itemize}
		\item $\sim 75\%$ of the EU buildings stocks is energy inefficient
		\item $\sim 40\%$ of the EU energy consumption is due to buildings
		\item Buildings are responsible for $\sim 36\%$ of the EU CO2 emissions
	\end{itemize}
\end{frame}
...
\begin{frame}{Context: Ktirio}
	\begin{figure}
		\centering
		\includegraphics[width=0.5\textwidth]{images/ktirio.png}
		\caption{Ktirio project \cite{img:ktirio}}
		\label{fig:figure1}
	\end{figure}

	\begin{itemize}
		\item Online platform for building energy simulation
		\item Provides tools for generating and simulating energy models
		\item Offers comfort and energy performance indicators
		\item Real-time monitoring of energy consumption/comfort
		\item Uses data assimilation to improve model reliability
		\item Incorporates model order reduction methods to speed up simulations
		\item Deployed on a cloud platform for access to computing resources
	\end{itemize}
\end{frame}
